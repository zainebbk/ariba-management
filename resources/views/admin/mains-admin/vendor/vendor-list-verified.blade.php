                                    ...
                                    <table class="table table-bordered mp-0 table-striped table-vcenter border-top text-nowrap" id="example1">
                                        ...
                                        @foreach($data['vendors'] as $vendor)
                                            <tr>
                                                ...
                                                <td><img class="w-7 h-7 rounded shadow mr-3" src="{{$imagesrc ?? ''}}" alt="{{$vendor->vendorname}}"></td>
                                                <td>{{$vendor->vendorname}}</td>
                                                <td>{{$vendor->display_name}}</td>
                                                <td>{{$vendor->store}}</td>
                                                <td><span class="badge badge-success mt-2">{{$vendor->totalproduct}}</span></td>
                                                <td>{{$vendor->email}}</td>
                                                <td>{{$vendor->date_added}}</td>
                                                <td class="align-middle">
                                                    <div class="btn-group align-top">
                                                        <button class="btn btn-sm btn-info" type="button"><a href="./update/{{$vendor->vendor_id}}" style="color:white">Edit</a></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
                    ...
                    <div class="modal-dialog modal-sm" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="smallmodal1">Delete</h5>
                            ...
                            <form action="" method="POST" id="delete-record">
                                @csrf
                                @method('DELETE')
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                    <button type="submit" class="btn btn-primary" id="delete-vendor">Delete</button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
                <!--end delete vendor modal -->
                ...
                <!-- INTERNAl Data tables -->
                    ...
                    <script>
                        $(document).on('click', '.delete', function(e) {
                            ...
                            $('#delete-record').attr('action', '/admin/vendor/delete/'+id);
                        });
                        ...
                        $(document).on('click', '.approve', function(e) {
                            ...
                            $.ajax({
                                ...
                                success:function () {
                                    var approve=$ele.parents("tr").find(".approve");
                                    var disapprove=$ele.parents("tr").find(".disapprove");
                                    var status=$ele.parents("tr").find(".status");
                        ...
                        $(document).on('click', '.disapprove', function(e) {
                            ...
                            $.ajax({
                                ...
                                success:function () {
                                    ...
                                    status.html("Waiting for approved");
