                    ...
                    <table class="table table-bordered table-hover text-nowrap">
                            ...
                            @foreach ($order_options as $order_option)
                                ...
                                @php $total+=($order_product->total+$order_product->tax*$order_product->quantity); @endphp
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{route("update-product",$order_product->product_id)}}">
                        ...
                        <tr>
                            <td colspan="9" class="text-right">
                                <a type="button"  @if($invoice) class="btn btn-warning not-allowed" onclick="return false;" @else class="btn btn-warning " @endif  href="{{route("invoice-create",[$order->order_id])}}" ><i class="si si-cloud-download"></i> Create</a>
                                <iframe src="{{route("invoice-pdf",[$order->invoice_id])}}" id="myframe" name="frame"  style="visibility: hidden;position: absolute;"></iframe>         
                                <button type="button" value="invoice" @if(!$invoice) class="btn btn-info printt disabled" disabled @else  class="btn btn-info printt" @endif() id="printt"><i class="si si-printer"></i> Print</button>
                                ...
                                <button type="button" @if($invoice) class="btn btn-secondary modal-effect disabled" disabled @else class="btn btn-secondary modal-effect" @endif()
            ...
            <div class="card-body">
                ...
                <div class="form-group m-0">
                    <label class="form-label">Selected Shipment Method: {{ $shipment->method_shipment }}.
                        @if ($shipment->method_shipment == 'Ariba Express')
                            @if ($shipment->courier_id)
                            <br><br>
                            Delivery Man: {{  $shiporder->courier->name }}
                            <button class="btn btn-primary"
                            data-effect="effect-scale" data-target="#deliverymanremove"
                            data-toggle="modal"
                            >Remove this Delivery Man</button>
                            @else
                            <button class="btn btn-success"
                                    data-effect="effect-scale" data-target="#deliveryman"
                                    data-toggle="modal"
                                    >Assign a Delivery Man</button>
                            @endif
                        @endif
                    </label>
...
<div class="modal" id="shipped">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{route('order-depot-shippe',$order->order_id)}}" method="post" class="needs-validation"
                    novalidate="">
                @csrf
...
<div class="modal  fade" id="smallmodal" tabindex="-1" role="dialog" aria-labelledby="smallmodal" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{route('order-depot-cancel',$order->order_id)}}" method="post">
                    @csrf
                <div class="modal-header">
...
<!--Confirm choice delivery method -->
<div class="modal fade" id="deliveryman" tabindex="-1" role="dialog" aria-labelledby="deliveryman" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{route('order-depot-deliveryman-ariba',$order->order_id)}}" method="post">
                    @csrf
                <div class="modal-header">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content modal-content-demo">
            ...
            <div class="modal-footer justify-content-center">
                <a class="btn btn-secondary" href="{{ route('order-depot-remove-delivery', $order->order_id) }}">Yes, Remove</a> <button class="btn btn-primary" type="button" data-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
</div>
...
<!-- Remove delivery method modal -->
<div class="modal fade" id="deliverymanremove" tabindex="-1" role="dialog" aria-labelledby="deliverymanremove" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title">Warning</h6><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
            ...
            <div class="modal-body">
                <p>Are you sure you want to remove this delivery man ?</p>
            </div>
            <div class="modal-footer justify-content-center">
                <a class="btn btn-secondary" href="{{ route('order-depot-deliveryman-ariba-remove', $order->order_id) }}">Yes, Remove</a> <button class="btn btn-primary" type="button" data-dismiss="modal">Close</button>
            </div>
        </div>
    </div>
</div>
