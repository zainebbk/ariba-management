...
@section('page-header')
    ...
    <div class="page-header">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">Order Shipped ( {{ count($orders) }} )</h4>
            <ol class="breadcrumb">
                                ...
                                <div class="table-responsive table-lg mt-3">
                                    <table
                                        ...
                                        <tr>
                                            ...
                                            <th class="wd-15p border-bottom-0">Order Nbr</th>
                                            ...
                                            <th class="wd-15p border-bottom-0">Delivery Methode</th>
                                        ...
                                        @foreach($orders as $order)
                                            <tr>
                                                ...
                                                <td><span class="badge badge-danger">{{$order->county ?? 0}}</span> </td>
                                                <td>@if ($order->from_app) App @else Website @endif</td>
                                                <td><span class="badge badge-warning">{{ $order->status }}</span></td>
                                                <td>{{ number_format(floatval($order->total+$order->tax),2) }} <small>Dh</small></td>
                                                ...
                                                <td class="align-middle">
                                                    <div class="btn-group align-top">
                                                        <button class="btn btn-sm btn-info ml-1"
                                                                ...
                                                                href="{{ route('order-depot-show-shipped', $order->order_id) }}"
                                                                style="color: white;padding-right:2px;">Show</a></button>
                                                        <button class="btn btn-sm btn-success"
                                                                data-id="{{$order->order_id}}"><a
                                                                href="{{route('order-depot-finish',$order->order_id)}}"
                                                                style="color: white" >Compelete</a></button>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endforeach
